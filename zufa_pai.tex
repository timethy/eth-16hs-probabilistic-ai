...
\begin{document}
	\begin{multicols*}{3}
		...
		\end{talign*}
		\emph{$X, Y$ conditional indepent given $Z$} iff $\uP(X, Y| Z) = \uP(X|Z) \uP(Y|Z)$. \\
...
%		\includegraphics[height=2cm]{img/pai1.png}
		...
		Variables $X_1, \cdots, X_n$. Pick order. For all $i \in [n]$ find 1.\ minimal subset $A \subseteq \{X_1,\dots,X_{i-1}\}$ s.t.\ $X_i \perp X_{\bar A}|X_A$.
		...
		Path is called \emph{active trail} from $X$ to $Y$ iff for all triples $A \rightarrow  B \rightarrow C$, $A \leftarrow  B \leftarrow C$,
		$A \leftarrow  B \rightarrow C$, $B$ is unobserved and for $A \rightarrow  B \leftarrow C$ either $B$ or one of its descendants is observed.
